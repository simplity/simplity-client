...
Service Agent is a utility that brokers services between a client-app and one or more server-apps that serve these services.
It abstracts a server-app for the client-app. Similarly, it abstracts a client-app for the server-app. This arrangement allows true de-coupling of the the apps. Also, it allows
...
The Service Agent on the client side provides a simple API for the client-side code to request a service without knowing the actual way in which the server implements the services. The Service Agent handles all the protocol related aspects of reaching out to the server, requesting the services with the relevant input data for that service and finally returning the response back to the code that initiated the request.
...
In a Service Based Architecture (SoA), the client-app is typically modelled as a set of pages that render certain data, either for just viewing or for editing. The requests are typically "get me data for so-and-so" and "update/create/delete so-and-so entity with attached data".

Simplity is typically used for data-intensive applications. And hence the Service Agent is built for SoA on the client-side. However, it can be used in RESTful paradigm as well because the API has all the features required for such an approach as well.
