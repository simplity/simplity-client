import { DEFAULT_MAX_CHARS, DEFAULT_MAX_NUMBER, DEFAULT_DAYS_RANGE, createValidationFn, } from './validation';
...
const maxDate = new Date(Date.UTC(nowYear, nowMon, nowDate + DEFAULT_DAYS_RANGE, 0, 0, 0, 0));
const minDate = new Date(Date.UTC(nowYear, nowMon, nowDate - DEFAULT_DAYS_RANGE, 0, 0, 0, 0));
const minDateMinus1 = new Date(Date.UTC(nowYear, nowMon, nowDate - DEFAULT_DAYS_RANGE - 1));
const maxDatePlus1 = new Date(Date.UTC(nowYear, nowMon, nowDate + DEFAULT_DAYS_RANGE + 1));
...
describe.each(Object.entries(testSets))('%s', (_desc, cases) => {
    describe.each(cases)('$description', ({ schema, okTests, notOkTests }) => {
        const fn = createValidationFn(schema);
        ...
        if (notOkTests && notOkTests.length) {
            it.each(notOkTests)('"$value" should be invalid', ({ value, errorId, params }) => {
                const res = fn(value);
                expect(res.value).toBeUndefined();
                const error = res.messages && res.messages[0];
                expect(error).toBeDefined();
                expect(error?.messageId).toBe(errorId);
                expect(error?.params).toStrictEqual(params);
            });
        }
    });
});
describe('Invalid ValueType', () => {
    it('should throw an error', () => {
        expect(() => {
            //@ts-expect-error
            createValidationFn({ valueType: 'invalid' });
        }).toThrow();
    });
});
